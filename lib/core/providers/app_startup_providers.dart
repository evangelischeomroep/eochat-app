...
/// EOchat fork hook: ensures a default server config exists when the fork is
/// configured to preconfigure the server. No-op when [ForkOverrides] skips the
/// setup screen (default), since the auth flow goes through
/// `ServerConnectionPage` and seeds storage itself.
...
class AppStartupFlow extends _$AppStartupFlow {
  ...
  void _scheduleForkBootstrap() {
    _scheduleAfterDelay(Duration.zero, () async {
      try {
  ...
  void _scheduleStartupProviderKeepAlives() {
    ...
    _scheduleDeferredKeepAlive(
      ...
      startupAuthStuckProvider,
      label: 'fork-startup-watchdog',
    );
    _scheduleAfterDelay(
