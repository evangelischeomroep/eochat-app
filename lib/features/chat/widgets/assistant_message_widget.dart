...
class _AssistantMessageWidgetState extends ConsumerState<AssistantMessageWidget>
  ...
  // Cache for _contentHasClearMarkdown — avoids re-running 13 regexes on
  // every ~100 ms streaming tick when the content hasn't actually changed.
  String? _markdownCheckContent;
  bool _markdownCheckResult = false;
  ...
  bool _contentHasClearMarkdown(String content) {
    ...
    // During streaming the content grows by a few chars per tick (~100 ms).
    // Skip the regex battery when the content is identical to the last check.
    if (content == _markdownCheckContent) {
      return _markdownCheckResult;
    }
    ...
    if (content.contains('```') ||
      ...
      result = _markdownBlockSyntaxPattern.hasMatch(content) ||
    ...
    _markdownCheckContent = content;
    _markdownCheckResult = result;
    return result;
  }
